...
const EditPackageModal: React.FC<EditPackageModalProps> = ({
  ...
  const [originalFormData, setOriginalFormData] = useState<PackageFormState>(initialFormState);
  const [allGroupCodes, setAllGroupCodes] = useState<GroupOption[]>([]);
  const [loading, setLoading] = useState(false);
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);

  // Fetch package data and group codes when modal opens
  useEffect(() => {
    if (!isOpen || packageid === null) return;

    setLoading(true);

    Promise.all([
      fetch(`/api/packagegroups/${packageid}`).then(res => res.json()),
      fetch("/api/packagegroups/groupcodes").then(res => res.json()),
    ])
      .then(([packageData, groupCodes]) => {
        // Handle multiple group codes
        const selectedGroupcodes = packageData.groupcodes && packageData.groupcodes.length > 0
          ? packageData.groupcodes.map((code: string) => ({ label: code, value: code }))
          : [];

        const packageFormData = {
          packageName: packageData.packagename || "",
          createDate: packageData.createdate ? packageData.createdate.split("T")[0] : "",
          comments: packageData.comments || "",
          groupcodes: selectedGroupcodes,
        };

        setFormData(packageFormData);
        setOriginalFormData(packageFormData);
        setAllGroupCodes(groupCodes);
      })
      .catch(() => Swal.fire("Error", "Failed to load package data.", "error"))
  ...
  const isDirty = useMemo(() => {
    const hasGroupcodesChanged = () => {
      if (formData.groupcodes.length !== originalFormData.groupcodes.length) return true;
      const sortedForm = formData.groupcodes.map(g => g.value).sort();
      const sortedOrig = originalFormData.groupcodes.map(g => g.value).sort();
      return sortedForm.some((val, idx) => val !== sortedOrig[idx]);
    };
    return (
      formData.packageName !== originalFormData.packageName ||
      formData.createDate !== originalFormData.createDate ||
      formData.comments !== originalFormData.comments ||
      hasGroupcodesChanged()
    );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        createdate: formData.createDate.split("T")[0],
      ...
      <div className="fixed inset-0 h-full w-full bg-gray-400/50 backdrop-blur-[32px] flex items-center justify-center z-50">
        <div className="bg-white dark:bg-gray-800 shadow-xl rounded-lg p-8">
          <div className="flex items-center justify-center">
            ...
            <span className="ml-3 text-gray-700 dark:text-gray-300">Loading package data...</span>
          </div>
        </div>
      </div>
    );
